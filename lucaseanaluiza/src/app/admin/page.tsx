...
export default function Admin() {
    ...
    async function reverterPresente(id: string) {
        const { error } = await supabase
            .from('presentes')
            .update({ disponivel: true, nome_escolhido: null })
            .eq('id', id)

        if (!error) {
            setPresentes(prev =>
                prev.map(p => p.id === id ? { ...p, disponivel: true, nome_escolhido: null } : p)
            )
        }
    }

    async function deletarConfirmacao(id: string) {
                ...
                <div className="flex gap-2 flex-wrap">
                    {[
                        ...
                        <button
                            ...
                            className={`font-josefin text-xs uppercase tracking-widest px-5 py-2 rounded-full border transition-colors ${aba === key
                ...
                {loading ? (
                    ...
                    <>
                                ...
                                {(aba === 'cha' ? confCha : confCasamento).length === 0 ? (
                                    ...
                                    (aba === 'cha' ? confCha : confCasamento).map(c => (
                                        <div key={c.id} className="rounded-2xl border border-[#F9CDBB] p-5 flex flex-col gap-1">
                                            ...
                                            {c.nomes_acompanhantes && c.nomes_acompanhantes.length > 0 && (
                                                <p className="font-josefin text-xs text-[#2D2D2D] opacity-50">
                                                    Acompanhantes: {c.nomes_acompanhantes.join(', ')}
                                                </p>
                                            )}
                        ...
                        {aba === 'presentes' && (
                            <div className="flex flex-col gap-3">
                                <div className="flex items-center justify-between">
                                    <p className="font-josefin text-xs uppercase tracking-widest text-[#2D2D2D] opacity-50">
                                        {presentesEscolhidos.length} presente{presentesEscolhidos.length !== 1 ? 's' : ''} escolhido{presentesEscolhidos.length !== 1 ? 's' : ''}
                                    </p>
                                </div>
                                    ...
                                    presentesEscolhidos.map(p => (
                                        <div key={p.id} className="rounded-2xl border border-[#CAD17A] p-5 flex items-center justify-between gap-3">
                                            <div>
                                                ...
                                                {p.nome_escolhido && (
                                                    <p className="font-josefin text-xs text-[#F26522] mt-1">por {p.nome_escolhido}</p>
                                                )}
                                            </div>
                                            <button
                                                onClick={() => reverterPresente(p.id)}
                                                className="font-josefin text-xs uppercase tracking-widest text-[#C42311] border border-[#C42311] px-3 py-1 rounded-full hover:bg-[#C42311] hover:text-white transition-colors"
                                            >
                                                Reverter
                                            </button>
                                        </div>
